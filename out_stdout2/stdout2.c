...
static int cb_stdout2_init(struct flb_output_instance *ins,
                          struct flb_config *config, void *data)
{
    int ret;
    const char *tmp;
    struct flb_stdout2 *ctx = NULL;
    (void) ins;
    (void) config;
    (void) data;

    ctx = flb_calloc(1, sizeof(struct flb_stdout2));
    if (!ctx) {
        flb_errno();
        return -1;
    }
...
static void cb_stdout2_flush(const void *data, size_t bytes,
                            ...
                            struct flb_config *config)
{
    ...
    size_t off = 0, cnt = 0;
    struct flb_stdout2 *ctx = out_context;
    flb_sds_t json;
    char *buf = NULL;
    (void) i_ins;
    (void) config;
...
static int cb_stdout2_exit(void *data, struct flb_config *config)
{
    struct flb_stdout2 *ctx = data;
...
static struct flb_config_map config_map[] = {
        ...
        },
        {
                ...
                0, FLB_TRUE, offsetof(struct flb_stdout2, json_date_key),
...
struct flb_output_plugin out_stdout2_plugin = {
        ...
        .cb_init      = cb_stdout2_init,
        .cb_flush     = cb_stdout2_flush,
        .cb_exit      = cb_stdout2_exit,
        .flags        = 0,
